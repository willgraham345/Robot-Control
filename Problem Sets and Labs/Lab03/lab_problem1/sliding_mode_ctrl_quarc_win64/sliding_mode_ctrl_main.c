...
typedef RT_MODEL_sliding_mode_ctrl_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * sliding_mode_ctrl(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(sliding_mode_ctrl_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: sliding_mode_ctrl -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
