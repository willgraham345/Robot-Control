...
ylabel("Y [m]")
xlabel("X [m]")
title({"Op-Space Inverse Dynamics Control", "End Effector Trajectory", "f = 1 circles/second"});
...
ylabel("Error [m]")
xlabel("Time [s]")
title({"Op-Space Inverse Dynamics Control", "XY Errors vs Time", "f = 1 circles/second"})
legend("X", "Y")
...
% ylabel("Error [deg]")
% xlabel("Time [s]")
% title({"Jacobian Inverse Control", "Joint Errors vs Time", "f = 1 circles/second"})
% legend("X", "Y")
% dim = [0.65, 0.15, 0.1, 0.1];
% t = annotation('textbox', dim, 'String', error_str, 'FitBoxToText','on', 'BackgroundColor', "w", "FontSize", 8)
% 
