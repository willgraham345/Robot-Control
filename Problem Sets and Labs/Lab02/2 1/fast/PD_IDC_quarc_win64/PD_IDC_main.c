...
typedef RT_MODEL_PD_IDC_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * PD_IDC(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(PD_IDC_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://PD_IDC:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: PD_IDC -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
