...
typedef RT_MODEL_PD_feed_forward_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * PD_feed_forward(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(PD_feed_forward_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: PD_feed_forward -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
