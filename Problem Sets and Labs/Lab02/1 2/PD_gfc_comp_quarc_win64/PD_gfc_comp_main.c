...
typedef RT_MODEL_PD_gfc_comp_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * PD_gfc_comp(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(PD_gfc_comp_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://PD_gfc_comp:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: PD_gfc_comp -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
