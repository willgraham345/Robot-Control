...
typedef RT_MODEL_JacInvCont_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * JacInvCont(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(JacInvCont_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://JacInvCont:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: JacInvCont -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
