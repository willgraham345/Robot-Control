...
typedef RT_MODEL_IDC_OS_lab_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * IDC_OS_lab(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(IDC_OS_lab_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://IDC_OS_lab:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: IDC_OS_lab -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
